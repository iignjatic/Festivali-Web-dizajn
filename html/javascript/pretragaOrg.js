...
let organizatori = {};
let form = document.getElementById("searchID");
...
form.addEventListener('submit', function(event) {
    ...
    let table1 = document.getElementById("allOrganizators1");
        table1.innerHTML = "";

    let table2 = document.getElementById("allOrganizators2");
        table2.innerHTML = "";    

    let naziv = document.getElementById("pretrazeno").value;



    getAllOrganizators(naziv);
});
});

function getAllOrganizators(naziv) {
    ...
    request.onreadystatechange = function () {
        if (this.readyState == 4) {
            if (this.status == 200) {
                ...
                organizatori = JSON.parse(request.responseText);        //parsira json
                for (let id in organizatori) {
                    let organizator = organizatori[id];                 //svaki organizator posebno

                    if (organizator.naziv.toLowerCase().includes(naziv.toLowerCase())) {       //ako se poklapa sa unesenim nazivom
                    ...
                    if (whichTable == 1) {
                        ...
                        appendOrganizatorRow("allOrganizators1", id, organizator, naziv);       //dodaje se u tabelu
                        }
                        else{
                        appendOrganizatorRow("izmjena1", id, organizator, naziv);
                        }
                    }
                    if (whichTable == -1) {
                        if(document.getElementById("allOrganizators2")!=null){
                        appendOrganizatorRow("allOrganizators2", id, organizator, naziv);
                        }
                        else{
                        appendOrganizatorRow("izmjena2", id, organizator, naziv);
                        }
                    }
...
function appendOrganizatorRow(tBody, id, organizator, naziv) {
    ...
    if (organizator.naziv.toLowerCase().includes(naziv.toLowerCase())) {
    let marked = document.createElement("mark");
    marked.textContent = organizator.naziv;
    marked.style.color = "green";

    nazivTd.appendChild(marked);
}
    }
    else{
